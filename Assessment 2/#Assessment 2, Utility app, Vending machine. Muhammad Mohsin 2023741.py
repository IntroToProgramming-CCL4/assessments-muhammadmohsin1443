...
def process_purchase(selected_item, money_inserted):
    if selected_item in item_codes:
        ...
        if money_inserted >= item_details['price']:
            ...
            print("Sorry, the inserted credit is insufficient. Please insert more money.")
    else:
        print("Dear customer, the item code inserted is invalid. Please enter a valid code.")
...
def suggest_extra_item(money_inserted):
    print("Would you like to buy an extra item?")
    extra_item = input("Enter 'yes' or 'no': ").lower()
    if extra_item == 'yes':
        print("Great choice Dear! ")
        ...
        print(f"We recommend trying {suggested_item} from the {suggested_category} category!")

        # Then asking the user if they want to add the suggested item
        add_suggested_item = input("Do you want to add this to your purchase? Enter 'yes' or 'no': ").lower()
        ...
        if add_suggested_item == 'yes':
            ...
            if suggested_item_code:
                ...
                money_inserted -= menu[suggested_category][suggested_item_code]['price']
                print(f"Remaining Balance: ${money_inserted:.2f}")
            else:
                print("Error: Item code not found for the suggested item.")
...
money_inserted = float(input("Enter money: $AED"))

# then we'll take the user input for the selection of items from our menu
selected_item = input("Enter the code of an item that you would want to purchase: ")
...
if transaction_completed:
    print("Thank you for using Al-Obese Vending Machine! Please collect your items below and change from the receipt box.")
    print("Follow us on our social media platforms: Instagram , Facebook , Twitter , Youtube , LinkedIn .")
else:
    print("Dear Customer the Transaction is failed. Please try again.")
...
for category, items in menu.items():
    for code, details in items.items():
        details['stock'] = 10  # 10 is Setted as an initial stock for each item
...
while True:
    ...
    if transaction_completed:
        print("Thank you for using Al Obese Vending Machine! Please collect your items and change.")
